...
eturkesRutils::datatable(dt) %>% DT::formatStyle("Genes", `text-align` = "center")
```
...
eturkesRutils::datatable(dt_spec) %>% DT::formatStyle("Genes", `text-align` = "center")
```
...
eturkesRutils::datatable(dt_abun) %>% DT::formatStyle("Genes", `text-align` = "center")
```
...
Due to the recency of its publication, we will benchmark the two techniques, along with the far simpler but still useful HM (half-minimum) technique.
...
This maxima is an arbitrary limit set by Proteome Discoverer and we remove it for our recalculations.
...
eturkesRutils::datatable(dt_qrilc) %>% DT::formatStyle("Genes", `text-align` = "center")
```
...
eturkesRutils::datatable(dt_hm) %>% DT::formatStyle("Genes", `text-align` = "center")
```
