...
```{r}
watson_direct_graph <- data.table::fread(
  "../assets/watson/relationshipgraph-bag3-co-ip-direct-to-bag3.csv"
)
watson_indirect_graph <- data.table::fread(
  "../assets/watson/relationshipgraph-bag3-co-ip-indirect-to-bag3.csv"
)
watson_direct_graph <- watson_direct_graph[
  , c("Source name", "Target name", "Confidence"), with = FALSE
]
...
co_ip_genes <- co_ip_genes[
  order(-co_ip_genes$`Watson Confidence`, -co_ip_genes$`Abundance Ratio: (scr/sh)`),
]

eturkesRutils::datatable(co_ip_genes) %>%
  DT::formatRound(c(2, 3)) %>% DT::formatStyle(cols[1], `text-align` = "center")
```
...
```{r}
tmp <- co_ip_genes[ , 1]
tmp$`Specificity: avg(Abun. Ratio, Watson)` <- co_ip_genes$`Abundance Ratio: (scr/sh)`
co_ip_genes <- cbind(tmp, co_ip_genes[ , 2:6])

for (row in 1:nrow(co_ip_genes)) {
  if (co_ip_genes[row, 4] > co_ip_genes[row, 3]) {
    co_ip_genes[row, 2] <- ((co_ip_genes[row, 4] + co_ip_genes[row, 3]) / 2 )
  }
}

co_ip_genes <- co_ip_genes[
  order(-co_ip_genes$`Specificity: avg(Abun. Ratio, Watson)`, co_ip_genes[[cols[4]]]),
]
