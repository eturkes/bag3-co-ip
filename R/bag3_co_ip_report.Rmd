...
co_ip_genes <- readRDS(file.path(getwd(), "..", "results", "data", "co_ip_genes_final.rds"))
...
watson_genes <- fread("../assets/watson/EntitySet-bag3-co-ip.csv")
...
![mydata-top-to-bottom-debug.png](../assets/debug/mydata-top-to-bottom-debug.png "Top to bottom debug flow leading to error")

#### Our data (left) compared to example data (right) at point of error
![left-mydata-right-untargeted.png](../assets/debug/left-mydata-right-untargeted.png "Our data (left) compared to example data (right) at point of error")
...
co_ip_genes_qrilc <- rownames_to_column(co_ip_genes_qrilc, var = "Genes")
...
co_ip_genes_hm <- rownames_to_column(co_ip_genes_hm, var = "Genes")
...
co_ip_genes_10min <- rownames_to_column(co_ip_genes_10min, var = "Genes")
...
co_ip_genes_cond_min <- rownames_to_column(co_ip_genes_cond_min, var = "Genes")
...
mvi_benchmark <- data.table("Missing Value Genes" = mvi_genes, "Original Rank" = 0, "QRILC Rank" = 0,
...
eturkesRutils::datatable(mvi_benchmark) %>% # From my own package, provides custom formatting.
  formatStyle("Missing Value Genes", `text-align` = "center")
```
...
ggplotly(ggplot(abun_ratio, aes(x = value, fill = variable)) +
  geom_density(alpha = 0.25) + xlab("Abundance Ratio: log2(scr/sh)"))
ggplotly(ggplot(abun_ratio, aes(x = value, fill = variable)) +
  geom_histogram(alpha = 0.25, bins = 10) + xlab("Abundance Ratio: log2(scr/sh)"))
```
...
![network-bag3-co-ip-direct-to-bag3.png](../assets/watson/network-bag3-co-ip-direct-to-bag3.png "Only Direct to BAG3")
...
![network-bag3-co-ip-indirect-to-bag3.png](../assets/watson/network-bag3-co-ip-indirect-to-bag3.png "Including Indirect to BAG3")
...
co_ip_genes <- co_ip_genes[order(
  -co_ip_genes$`Watson Confidence`, -co_ip_genes$`Abundance Ratio: (scr/sh)`), ]
eturkesRutils::datatable(co_ip_genes) %>% # From my own package, provides custom formatting.
...
co_ip_genes <- co_ip_genes[order(
  -co_ip_genes$`Specificity: avg(Abun. Ratio, Watson)`, co_ip_genes[[cols[4]]]), ]
eturkesRutils::datatable(co_ip_genes) %>% # From my own package, provides custom formatting.
...
```{r}
saveRDS(co_ip_genes, file = file.path(results_dir, "data", "co_ip_genes_final.rds"))
```
