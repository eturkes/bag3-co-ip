...
```{r}
dt < dt_10min
scale_factor <- 100 / max(dt[[cols[2]]])
dt[[cols[2]]] <- dt[[cols[2]]] * scale_factor
colnames(dt)[2] <- "Abundance Ratio: (scr/sh)"
...
ibb_out <- ibb::ibb.test(
  dt[ , 3:4], colSums(dt[ , 3:4]), c(rep("pre", 1), rep("post", 1)), n.threads = -1
)
dt <- cbind(dt, ibb_out$p.value)
colnames(dt)[5] <- "p-value"
...
```{r}
saveRDS(dt, file = "../results/dt_final.rds")
