...
watson_direct_graph <- watson_direct_graph[
  , c("Source name", "Target name", "Confidence", "Documents"), with = FALSE
]
...
These must be combined for comparison with the Co-IP data, which does not distinguish the direction of association.
We use the below custom equation to do so.

$
\begin{equation*}
Confidence_{combined} = \frac{(Confidence_{down} \times Documents_{down}) + (Confidence_{up} \times Documents_{up})}{(Documents_{down} + Documents_{up})}
\end{equation*}
$

```{r}
watson_spread <- watson_direct_graph[
  grepl("BAG3", watson_direct_graph$`Source name`) |
    grepl("BAG3", watson_direct_graph$`Target name`),
]

#for (gene in 1:length(uniq_genes))
```
