...
cols <- c("Genes", "Abundance Ratio", "scrRNA Abundance", "shBAG3 Abundance")
...
ggplotly(ggplot(abun_ratio, aes(x = value, fill = variable)) +
  geom_histogram(alpha = 0.25, bins = 10) + xlab("Abundance Ratio"))
```
...
For easier analysis and interpretation, we have decided to renormalize the values in the `Abundance Ratio` column.
...
colnames(co_ip_genes)[2] <- "Abundance Ratio"
eturkesRutils::datatable(co_ip_genes) %>% # From my own package, provides custom formatting.
  formatRound(2) %>% formatStyle(cols[1], `text-align` = "center")
```
...
co_ip_genes <- co_ip_genes[order(
  -co_ip_genes$`Watson Confidence`, -co_ip_genes$`Abundance Ratio`), ]
eturkesRutils::datatable(co_ip_genes) %>% # From my own package, provides custom formatting.
...
# Initialize data table.
tmp <- co_ip_genes[ , 1]
tmp$`BAG3 Specificity` <- co_ip_genes$`Abundance Ratio`
