...
cols <- c(
  "Gene", "BAG3 Specificity", "Abundance Ratio", "Watson Confidence",
  "Watson Similarity", "scrRNA Abundance", "shBAG3 Abundance")
```
...
![](../assets/watson/predictive-tree.png)

### Distance Network

![](../assets/watson/predictive-distance.png)

### Ranking Table

```{r}
predictive <- fread(file.path(assets_dir, "watson", "predictive-neighborhoodgraph.csv"))
predictive_cols <- c("Gene", "Set", "Similarity")
predictive <- plyr::rename(predictive, c( # Resolve masking conflict.
  "Display name" = predictive_cols[1], "Similarity score" = predictive_cols[3]))
predictive <- predictive[ , predictive_cols, with = FALSE]
predictive <- predictive[-nrow(predictive), ] # Remove empty last row.
datatable(predictive) %>%
...
# Initialize data table.
tmp <- co_ip_genes[ , 1:4]
tmp$`Watson Similarity` <- 0
co_ip_genes <- cbind(tmp, co_ip_genes[ , 5:7])

# Add the scores.
for (gene in 1:nrow(predictive)) {
  for (row in 1:nrow(co_ip_genes)) {
    if (co_ip_genes[row, 1] == predictive[gene, 1]) {
      co_ip_genes[row, 5] <- predictive[gene, 3]}}}

# Normalize the scores.
co_ip_genes$`Watson Similarity` <- co_ip_genes$`Watson Similarity` * 100
